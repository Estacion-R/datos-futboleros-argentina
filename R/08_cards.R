...
source(file.path(
  ...
  "R/paleta_estacion_r.R"
)) # secuencial oficial (B)
...
cents <- sf::st_centroid(mapa_data[
  mapa_data$jugadores >= 1 &
    mapa_data$nombre != "Ciudad Autónoma de Buenos Aires",
])
cents_xy <- cbind(
  as.data.frame(sf::st_coordinates(cents)),
...
p_main_base <- ggplot(mapa_data) +
  ...
  geom_text(
    data = lab_nac,
    aes(X, Y, label = jugadores, color = col_txt),
    family = F_TIT,
    fontface = "bold",
    size = 6
  ) +
...
p_zoom <- ggplot(metro) +
  ...
  fill_scale +
  coord_sf(
    xlim = c(ins_box$xmin, ins_box$xmax),
    ylim = c(ins_box$ymin, ins_box$ymax),
    expand = FALSE
  ) +
...
p_mapa <- (p_main | p_zoom) +
  plot_layout(widths = c(1.5, 1)) +
  plot_annotation(
    title = paste0("¿Orígen de los convocados"),
    subtitle = "Jugadores del plantel según provincia de nacimiento",
    caption = paste0(
      "Los ",
      por_provincia$jugadores[por_provincia$provincia == "Buenos Aires"],
      " son de la PROVINCIA de Buenos Aires; de la Ciudad (CABA), ninguno.\n",
      sum(plantel$pais != "Argentina"),
      " nacieron en el exterior: Nico Paz (España) y G. Simeone (Italia)."
    ),
    theme = theme(
      plot.title = element_text(
        family = F_TIT,
...
build_mapa_highlight <- function(provincias_hl = character(0), subtitulo = "") {
  ...
  con_jug <- d[
    d$jugadores >= 1 & d$nombre != "Ciudad Autónoma de Buenos Aires",
  ]
  ctrs <- cbind(
...
save_card(
  build_mapa_highlight(
    subtitulo = "Las provincias en azul tienen al menos un jugador"
  ),
  "01_mapa_base.png",
  h = 1080
)
...
save_card(
  build_mapa_highlight(
    c("La Pampa", "Entre Ríos", "San Luis", "Tucumán"),
    "La Pampa, Entre Ríos, San Luis y Tucumán: 1 cada una"
  ),
  "01_mapa_hl_resto.png",
  h = 1080
)
...
p_caps <- ggplot(pl_caps, aes(caps, name, fill = es_messi)) +
  ...
  scale_fill_manual(
    values = c(`FALSE` = ER_AZUL, `TRUE` = ER_NEGRO),
    guide = "none"
  ) +
  scale_x_continuous(expand = expansion(mult = c(0, 0.12))) +
  labs(
    title = "La mochila de la experiencia",
